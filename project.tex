...
%%%% show a plot with intermittent features

%%% histogram of the ellipse measures here

\begin{figure}\label{fig:poincare_plots}
  \begin{center}
    %%\includegraphics[scale=0.6]{poincare_1}
    %%\includegraphics[scale=0.6]{poincare_2}
    %%\includegraphics[scale=0.6]{poincare_3}
    %%\includegraphics[scale=0.6]{poincare_4}
  \end{center}
...
Although it would be possible to project the points into the 3-space for the sliding window trigrams of the time series under study and generalize the Poincare plot this way, we attempted instead a more ad hoc approach to studying the progression of the Poincare plot as more points were added.

%%% why?
%%% how?
%%% what's wrong with it?
%%% what can you reason with it?
%%% why can't you actually reason with it?
...
%%% example of time series which is caught by mutual information but not by correlation
...
A measure of synchronization in the frequency domain is coherence, which is also a linear measure of synchronization, just not in the time basis. It is different from normal lagged correlation because %%% what different properties does it have
...
This determines uniquely the unwrapped phase that we were looking for, but only upon the assumption that this is a narrow-band signal, meaning one without too much frequency in a broad band. In our data, this may not be the case, so this analysis must also be taken with a grain of salt. %%%%% graph a spectrum of our fbm, refer that shit
...
%%%% reconstructed stack graph, note distortions from bucketing
