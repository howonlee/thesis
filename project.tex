...
\begin{figure}\label{nonlinearity}
  %%%% add the nonlinearity figure here
\end{figure}

One example which should help develop a theoretical intuition about nonlinearity and synchronization is the sine wave relation example. Consider the figure \ref{nonlinearity}. According to a linear correlation measure, taking the whole domain of $x$ into account at once, the two variables $x$ and $y$ are not related to each other, but by inspection, one can say that actually $y = 1 - x^2$. But nonlinear measures \emph{will} take this nonlinear relation into account. For example, mutual information will say that the mutual information between the two variables $x$ and $y$ is %%%% whatever the mutual information is.

There exists a linear \emph{spectral} measure, coherence, which has been used in relating time series data before.
