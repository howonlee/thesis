...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% 3 citations each, k?
Lagged correlation is the most common measure of synchrony in social science datasets. We will examine those in this project as a first pass as well as recurrence methods, spectral coherence, Hilbert transforms and transformation into networks. %%% citations
...
Because we are dealing with real data, mutual information must be estimated from probability distributions represented as histograms. This is problematic in high-dimensional data, owing to the curse of dimensionality\cite{bellman}. Also, it is the case that the measure itself depends upon an intelligent discretization. %%%% Therefore, as according to \the alzheimers paper, which held the buckets for neurological data, we will bucket in the exact same way those kids did
...
%%% talk about the Fourier basis claims of the visibility graph lacasa paper, recapitulate it, wonder about it
...
In order to reconstruct the time series from a multigraph of this kind, therefore, we start from the start node again and consume the adjacency lists starting from the initial elements of the lists, to find the next node to jump to. The model is very reminiscent of models from symbolic dynamics %%%% cite models from symbolic dynamics, like the campanharo paper did

%%%% %%%%%%%%%%%%%%%%
%%%% %%%%%%%%%%%%%%%%
%%%% bucketing matters, cite the alzheimers stuff again
...
\begin{figure}\label{fig:stack_ts_spectra}
  \begin{center}
    ...
    \includegraphics[scale=0.6]{spectrum_vr}
  \end{center}
...
\begin{figure}\label{fig:stack_ts_degree}
  \begin{center}
    ...
    \includegraphics[scale=0.6]{spectrum_vr}
  \end{center}
  \caption{Degree distribution in stack graph. From left to right: sinusoid, logistic map with full chaos (r=3.8), fractional Brownian motion, real data}
\end{figure}
